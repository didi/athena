...
Status CTCLossCalculator<T>::CalculateLoss(
  ...
  auto ComputeLossAndGradients = [this, num_classes, &labels, &l_primes,
    ...
    for (int b = start_row; b < limit_row; b++) {
      ...
      // prediction. Do lazy evaluation of log_prob here.（原论文为log(p(z|x)）
...
void CTCLossCalculator<TT>::CalculateForwardVariables(
  ...
  for (int t = 1; t < T; ++t) {
    ...
    // be kLogZero.
    for (int u = std::max(0, U - (T - t)); u < std::min(U, (t + 1));//菱形的范围需要改（缩小2倍）
         ++u) {
      ...
      if (t >= 2 && u > 0) {
        sum_log_alpha = LogSumExp(sum_log_alpha, log_alpha->coeff(u - 1, t - 1));
      }
...
void CTCLossCalculator<TT>::CalculateBackwardVariables(
  ...
  for (int u = U - 1; u < U; ++u) log_beta->coeffRef(u, T - 1) = 0;
